...
test("parseFlowerConfig accepts development config and normalizes base URL", () => {
  const config = parseFlowerConfig(
    {
      apiBaseUrl: "https://mitsubachi-api.shiosalt.com/",
      developmentAccessToken: "secret",
...
test("production config rejects developmentAccessToken", () => {
  assert.throws(
    () => parseFlowerConfig({ apiBaseUrl: "https://mitsubachi-api.shiosalt.com", developmentAccessToken: "secret" }, "development.json", "production"),
...
test("redactSecrets removes tokens and authorization values", () => {
  const value = redactSecrets(
    'Authorization: Bearer abc developmentAccessToken":"secret" access_token":"tok-secret" device_code":"dev-secret" https://x.test/file?token=abc&ok=1',
  );
